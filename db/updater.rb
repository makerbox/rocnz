...
calldb("wholesaleportal")

def calldb(db)
	require 'rdbi-driver-odbc'
	puts "connecting to database"
	dbh = RDBI.connect :ODBC, :db => db
	self.products = dbh.execute("SELECT * FROM product_master").fetch(:all, :Struct)
	self.productsext = dbh.execute("SELECT * FROM prodmastext").fetch(:all, :Struct)
	self.customers = dbh.execute("SELECT * FROM customer_master").fetch(:all, :Struct)
	self.activecustomers = dbh.execute("SELECT * FROM customer_mastext").fetch(:all, :Struct)
	self.contacts = dbh.execute("SELECT * FROM contact_details_file").fetch(:all, :Struct)
end
