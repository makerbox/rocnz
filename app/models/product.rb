...
def calc_discount(user, price, prod_group, prod_code, price_cat)
	if Discount.where(product: (prod_group || prod_code || price_cat), customer: (user.account.code.strip || user.account.discount)).exists?
		disco = Discount.where(product: (prod_group || prod_code || price_cat), customer: (user.account.code.strip || user.account.discount)).first
